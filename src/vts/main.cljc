(ns vts.main
  "Kotodama WASM entrypoint for the vts clean-room actor (L4) — Clojure port.
  ...
  facts (`vts.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"vts.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "vts-compat" :tier tier :entities entities} 200])
